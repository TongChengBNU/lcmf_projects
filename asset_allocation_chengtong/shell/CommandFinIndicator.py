...
from sqlalchemy import MetaData, Table, Column, select, insert, delete, func, literal_column, ForeignKey
from sqlalchemy import Integer, Date, Numeric, Float, String
...
def pool_rank(ctx):
    ...
    for i in range(len(df)):
        ins = asset_allocate.fund_pool(
            fp_date = df.index[i],
            fp_statistics_type = df.iloc[i, 0],
            fp_average_return_bond = df.iloc[i, 1],
            fp_rank_bond = df.iloc[i, 2],
            fp_average_return_stock = df.iloc[i, 3],
            fp_rank_stock = df.iloc[i, 4],
            fp_average_return_currency = df.iloc[i, 5],
            fp_rank_currency = df.iloc[i, 6]
                )
        session.add(ins)
        session.commit()
        print("成功插入了 %d 条数据;" % (i+1))
...
def jp_huobi_update(ctx):
    ...
    df_jijin = pd.read_sql(sql=sql_jijin, con=session.bind, parse_dates=['jh_date'], index_col=['jh_date'])
    df_qieman = pd.read_sql(sql=sql_qieman, con=session.bind, parse_dates=['jh_date'], index_col=['jh_date'])
    session.commit()
    session.close()
    dfCore = pd.merge(left=df_jijin, right=df_qieman, on='jh_date', how='inner')
    ...
    for count, index in enumerate(dfCore.index):
        ins = asset_allocate.jp_huobi(
                    ...
                    jh_week_annualized_return_qieman = dfCore.loc[index, dfCore.columns[2]],
