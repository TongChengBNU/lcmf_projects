...
# def crossData(date):
#     codeSet = Code(date)
#     df_assets_slice = df_assets.loc[df_assets.S_INFO_WINDCODE.apply(lambda x: x in codeSet)].copy()
#
#     if len(codeSet) < 2:
#         sql_t = "SELECT F_INFO_WINDCODE, ANN_DATE, PRICE_DATE, F_NAV_ADJUSTED FROM chinamutualfundnav WHERE F_INFO_WINDCODE IN " + \
#                 str(tuple(codeSet)).replace(',', '') + ";"
#     else:
#         sql_t = "SELECT F_INFO_WINDCODE, ANN_DATE, PRICE_DATE, F_NAV_ADJUSTED FROM chinamutualfundnav WHERE F_INFO_WINDCODE IN " + \
#                 str(tuple(codeSet)) + ";"
#
...
#         'F_INFO_WINDCODE': 'S_INFO_WINDCODE',
#         'PRICE_DATE': 'F_PRT_ENDDATE',
#         'ANN_DATE': 'F_ANN_DATE'
#     }, inplace=True)
#
#     dfCore = pd.merge(left=df_assets_slice, right=df_nav, on=['S_INFO_WINDCODE', 'F_PRT_ENDDATE', 'F_ANN_DATE'], how='inner', sort=True)
#     dfCore.set_index(pd.DatetimeIndex([pd.Timestamp(2015, 1, 2) for i in range(len(dfCore))]), inplace=True)
#
#     return  dfCore
# def main():
#     df = pd.DataFrame()
#     # set_trace()
#     for date in trade_months:
#         tmp_df = crossData(date)
