...
def crossData_df_2(params: dict) -> pd.DataFrame:
    date = params['date']
    try:
        sortKey = params['sortKey']
    except Exception:
    ...
    df_rank_month = pd.DataFrame(columns=['S_INFO_WINDCODE', 'F_PRT_BONDTOTOT', 'F_PRT_FOUR', 'RETURN'])
    # df_month.F_PRT_CP_PERCENTAGE = df_month.F_PRT_CPVALUE / df_month.F_PRT_TOTALASSET * 100
    # df_month.F_PRT_MTN_PERCENTAGE = df_month.F_PRT_MTNVALUE / df_month.F_PRT_TOTALASSET * 100
    # df_month.F_PRT_CORP_PERCENTAGE = df_month.F_PRT_CORPBOND / df_month.F_PRT_TOTALASSET * 100
    ...
    df_rank_month.S_INFO_WINDCODE = df_month.S_INFO_WINDCODE
    df_rank_month.F_PRT_BONDTOTOT = df_month.F_PRT_BONDTOTOT
    df_rank_month.F_PRT_FOUR = df_month.F_PRT_CPVALUE / df_month.F_PRT_TOTALASSET * 100 + df_month.F_MMF_REVERSEREPO / df_month.F_PRT_TOTALASSET * 100 + df_month.F_PRT_CDS / df_month.F_PRT_TOTALASSET * 100 + df_month.F_PRT_CASHTOTOT
    df_rank_month.RETURN = df_month.RETURN
    ...
    #df_total_25, df_total_50, df_total_75 = df_rank_month.F_PRT_BONDTOTOT.describe()[4:7].values
    #df_20_return = df_rank_month.RETURN.sort_values(ascending=False).iloc[int(len(df_rank_month) * 0.2)]
    #df_20_return = df_rank_month.RETURN.dropna().quantile(q=0.2)

    df_20_return = df_rank_month.RETURN.dropna().quantile(q=0.8)

    dfCore = pd.DataFrame(index=[date], columns=['portfolio', '20'])
    #dfCore.loc[date, 'portfolio_bond_mean'] = df_portfolio_bond
    dfCore.loc[date, 'portfolio'] = df_portfolio_return * 1200
    #dfCore.loc[date, 'total_25'] = df_total_25
    #dfCore.loc[date, 'total_50'] = df_total_50
    #dfCore.loc[date, 'total_75'] = df_total_75
    dfCore.loc[date, '20'] = df_20_return * 1200

    return dfCore



def crossData_df_3(params: dict) -> pd.DataFrame:
    date = params['date']
    try:
        sortKey = params['sortKey']
    except Exception:
    ...
    df_month = pd.read_csv("./Data/df_month-"+str(date)[:10]+".csv", parse_dates=[0], index_col=[0])
    df_rank_month = pd.DataFrame(columns=['S_INFO_WINDCODE', 'F_PRT_BONDTOTOT', 'F_PRT_FOUR', 'RETURN'])
    # df_month.F_PRT_CP_PERCENTAGE = df_month.F_PRT_CPVALUE / df_month.F_PRT_TOTALASSET * 100
    # df_month.F_PRT_MTN_PERCENTAGE = df_month.F_PRT_MTNVALUE / df_month.F_PRT_TOTALASSET * 100
    # df_month.F_PRT_CORP_PERCENTAGE = df_month.F_PRT_CORPBOND / df_month.F_PRT_TOTALASSET * 100
...
@click.pass_context
@click.option("--date", help='timestamp')
def crossData(ctx, date, sortKey = 'F_PRT_BONDTOTOT') :
    ...
    return df_rank_month



@cashportfolio.command()
@click.pass_context
@click.option('--path', 'save_path', prompt='Path for output:', help='Path for .csv output.')
@click.option('--sortKey', 'sortKey', prompt='Option 1: F_PRT_BONDTOTOT; Option 2: F_PRT_THREE;->', help='Columns to sort in ranking;')
def main(ctx, save_path, sortKey):
    try:
        file1 = open(file=save_path, mode='w')
        file1.close()
    except FileNotFoundError:
        logging.warning("Path Error;")
        return


    dfCore = pd.DataFrame(index=trade_months, columns=['portfolio', '20'])
    with click.progressbar(dfCore.index, label='cashPortfolio') as bar:
        for index in bar:
            try:
                #portfolio_bond, portfolio_return, total_25, total_50, total_75, total_return  = crossData(date=index, sortKey=sortKey)
                #dfCore.loc[index, 'portfolio_bond_mean'] = portfolio_bond
                #dfCore.loc[index, 'portfolio'] = portfolio_return
                #dfCore.loc[index, 'total_25'] = total_25
                #dfCore.loc[index, 'total_50'] = total_50
                #dfCore.loc[index, 'total_75'] = total_75
                #dfCore.loc[index, 'total'] = total_return
                params = {'date': index, 'sortKey': sortKey}
                df_query = crossData_df_2(params) 
                dfCore.loc[index, 'portfolio'] = df_query.iloc[0,0]
                dfCore.loc[index, '20'] = df_query.iloc[0,1]
            except IndexError:
                logging.warning("Error Date: %s" % str(index))
            except sqlalchemy.exc.OperationalError:
                logging.warning("Mysql Connection Error: %s" % str(index))

    dfCore.to_csv(save_path)
...
#@click.option('--path', 'save_path', prompt='Path for output:', help='Path for .csv output.')
#@click.option('--sortKey', 'sortKey', prompt='Option 1: F_PRT_BONDTOTOT; Option 2: F_PRT_THREE;->', help='Columns to sort in ranking;')
#def multiprocess_ct(ctx, sortKey, save_path):
def multiprocess_ct(sortKey, save_path):
    pool = multiprocessing.Pool(min(multiprocessing.cpu_count()//2, 6))
    params = [{'date': d, 'sortKey': sortKey} for d in trade_months]
    res = pool.map(crossData_df_2, params)
    pool.close()
    pool.join()
    df = pd.concat(res)
    df.to_csv(save_path)
